...
<c:set var="buttonColor" value="${settings.buttoncolor}" />
<c:set var="teaserLength" value="${settings.teaserlength}" />
...
<div class="row mb-5">
    <div class="col-xs-12">
        <h3 class="mb-5">
            <a href="<cms:link baseUri="${settings.pageUri}">${filename}</cms:link>">${headline}</a>
        </h3>
        <c:set var="showdate"><c:out value="${settings.showDate}" default="true" /></c:set>
        ...
        <a href="<cms:link baseUri="${settings.pageUri}">${filename}</cms:link>" class="btn ap-btn-xs ap-btn-${buttonColor}">
