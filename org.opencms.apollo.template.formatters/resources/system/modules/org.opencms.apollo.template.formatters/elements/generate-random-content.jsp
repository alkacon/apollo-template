...
<c:choose>
	<c:when
		test="${not empty fn:trim(param.count) and not empty fn:trim(param.path)and not empty fn:trim(param.ext) and not empty fn:trim(param.prefix)}">
		    ...
		    String targetFolderSitePath = request.getParameter("path");
							...
							if (!cms.existsResource(targetFolderSitePath)) {
								...
								cms.createResource(targetFolderSitePath,
										...
										null, new ArrayList<CmsProperty>(0));
							}
							for (int i = 1; i <= fileCount; i++) {
								...
								String creationPath = targetFolderSitePath + "/" + prefix + "_"
										+ String.format("%04d", Integer.valueOf(i)) + "." + ext;
								if (!cms.existsResource(creationPath)) {
									out.println("Creating file " + creationPath + "<br />\n");
									CmsResource createdResource = cms.createResource(creationPath, OpenCms.getResourceManager()
...
<link
	href="<cms:link>/system/modules/org.opencms.apollo.template.basics/resources/css/styles-main.min.css</cms:link>"
	rel="stylesheet" />
<link rel="stylesheet"
	href="<cms:link>/system/modules/org.opencms.apollo.template.basics/resources/css/style-red.min.css</cms:link>">
<style>
.centered {
	margin: 0 auto;
}
...
<body class="wrapper">
	<div class="row">
		<div class=" mt-10 col-md-4 col-centered text-center">
			<div class="mt-20 opencms-logo"></div>
			<h1>Random content generator</h1>
			<form class="service-block service-block-grey row" method="get">
				...
				<input class="mb-10 form-control col-xs-12" type="text" name="path"
					placeholder="Path to files (recursive to active site)"
					required="required" required="required" /> <input
					class="mb-10 form-control col-xs-12" type="text" name="prefix"
					placeholder="Prefix of files" required="required"
					required="required" /> <input class="mb-10 form-control col-xs-12"
					type="text" name="ext" placeholder="file extension of files"
					required="required" />
				<button type="submit" class="col-xs-12 btn btn-default submit">Submit</button>
			</form>
			<div id="log"></div>
		</div>
	</div>
	<script
		src="<cms:link>/system/modules/org.opencms.apollo.template.basics/resources/js/scripts-all.min.js</cms:link>"></script>
