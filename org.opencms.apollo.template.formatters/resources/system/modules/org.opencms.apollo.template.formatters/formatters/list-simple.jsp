	...
	<cms:formatter var="con" val="value" rdfa="rdfa">
		...
		<div class="ap-list-content mb-20">
			...
			<apollo:init-messages textnew="${textnew}" textedit="${textedit}">
                ...
                <div class="${cms.element.setting.wrapperclass.isSet ? cms.element.setting.wrapperclass : '' }" id="list-${cms.element.instanceId}">
				<%-- ####### List entries ######## --%>

				<c:set var="itemCount" value="${value.ItemsPerPage.isSet ? value.ItemsPerPage.toInteger : 5}" />
                <c:set var="wrapItem" value="${(cms.element.setting.wrapperclass.isSet and fn:contains(cms.element.setting.wrapperclass.value, 'panel')) ? 'false' : 'true' }" />
				<apollo:list-main 
						source="${value.Folder}" 
