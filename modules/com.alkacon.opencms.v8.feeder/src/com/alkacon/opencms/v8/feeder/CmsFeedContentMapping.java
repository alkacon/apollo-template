/*
 * File   : $Source: /alkacon/cvs/alkacon/com.alkacon.opencms.v8.feeder/src/com/alkacon/opencms/v8/feeder/CmsFeedContentMapping.java,v $
...
public class CmsFeedContentMapping {
    ...
    class CmsFeedMacroResolver extends CmsMacroResolver {
         ...
         */
        public CmsFeedMacroResolver(CmsObject cms, CmsXmlContent content, Locale locale) {
    ...
    private static final Log LOG = CmsLog.getLog(CmsFeedContentMapping.class);
        ...
        if (hasTitle && hasDescription) {
            ...
            if (link == null) {
                ...
                link = OpenCms.getLinkManager().getServerLink(
                    cms,
                    macroResolver.getMacroValue(
                        CmsFeedMacroResolver.MACRO_XML + "_" + CmsFeedMacroResolver.MACRO_URI + "_"));
            }
    ...
    public List getRssEntries(CmsObject cms, CmsXmlContent content, Locale locale) {
        ...
        if ((content == null) || (locale == null) || !content.hasLocale(locale)) {
            ...
            LOG.debug(
                "RSS feed returns empty list, because"
                    + (content == null ? " content is null" : "")
                    + (locale == null ? " locale is null" : "")
                    + ((content != null) && (locale != null) && !content.hasLocale(locale)
                    ? " content does not have locale " + locale
                    : ""));
            return entries;
        }
        // indicate we always want to use full links in the generated HTML output
        cms.getRequestContext().setAttribute(CmsRequestContext.ATTRIBUTE_FULLLINKS, Boolean.TRUE);
        LOG.debug("Using data type " + (m_dataType != null ? m_dataType : "null (this is good)"));
        try {
            if (m_dataType == null) {
                CmsMacroResolver rssMacroResolver = new CmsFeedMacroResolver(cms, content, locale);
                LOG.debug(
                    "Initialized macro resolver \""
                        + (rssMacroResolver != null ? rssMacroResolver.getClass().getName() : "which is null (bad!)")
                        + "\" for content, now trying to get entry...");
                SyndEntry entry = getEntry(cms, rssMacroResolver);
                LOG.debug(
                    "RSS Feed processing entry " + (entry != null ? entry.getUri() : "entry is null (this is bad!)"));
                if (entry != null) {
                    entries.add(entry);
                    LOG.debug("RSS Feed added entry " + entry.getUri());
                }
            }
        } catch (Exception e) {
