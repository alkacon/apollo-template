...
<c:choose>
    <c:when test="${(resTypeName == 'a-templaterow') && (typeName == 'mainsection')}">
        <c:set var="variant" value="jsp" /> 
    </c:when> 
    <c:when test="${(resTypeName == 'a-templaterow') && !(typeName == 'default')}">
...
<div class="oc-container-${variant}">
    <h1><fmt:message key="apollo.row.headline.emptycontainer"/> 
