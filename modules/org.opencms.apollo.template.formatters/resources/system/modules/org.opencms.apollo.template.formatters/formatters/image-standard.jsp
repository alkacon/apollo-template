				...
				<div class="${cssClass}" >
					...
					<c:choose>
						<c:when	test="${imgValParent.Image.value.Description.isSet and cms.element.setting.showtext.value == 'true'}">
							<p class="margin-top-10" ${imgValParent.Image.rdfa.Description}>${imgValParent.Image.value.Description}</p>
						</c:when>
						<c:when	test="${imgValParent.Text.isSet and cms.element.setting.showtext.value == 'true'}">
							<p class="margin-top-10" ${imgValParent.Text.rdfaAttr}>${imgValParent.Text}</p>
						</c:when>
					</c:choose>
				</div>
			</c:otherwise>
